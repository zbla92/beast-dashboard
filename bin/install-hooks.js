#!/usr/bin/env node
'use strict';
// Adds the Beast Dash hook to Claude Code's settings (both accounts), idempotently. Re-run any time.
...
const FILES = [path.join(HOME, '.claude', 'settings.json'), path.join(HOME, '.claude-company', 'settings.json')];
