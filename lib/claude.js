...
async function refreshPeeks(names) {
  await Promise.all(names.map(async name => {
    ...
    peeks.set(name, { lines: shown, draft, paneState, limitText, at: now, since: prev && prev.paneState === paneState ? prev.since : now });
...
function info(name, activity) {
  ...
  return {
    ...
    account: r?.account || null, sessionId: r?.sessionId || null, title: r?.title || null, model: r?.model || null,
    lastTool: r?.lastTool || null, lastPrompt: r?.lastPrompt || null, lastMessage: r?.lastMessage || null, lastEventAt: r?.lastEventAt || 0,
    peek: p ? p.lines : [], draft: p ? p.draft : '', hooked: !!(r && r.lastEventAt),
