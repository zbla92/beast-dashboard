...
function list(root, rel, changed) {
  ...
  for (const e of ents) {
    ...
    if (isDir) {
      ...
      let mtime = 0; try { mtime = fs.statSync(path.join(t.abs, e.name)).mtimeMs; } catch {}
      out.push({ name: e.name, type: 'dir', path: relp, changed: n, skip: SKIP.has(e.name), mtime });
    } else {
...
// inline = show it in the browser (image / PDF preview, "open in a tab") instead of saving it
const INLINE = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp', '.svg': 'image/svg+xml', '.pdf': 'application/pdf', '.txt': 'text/plain; charset=utf-8', '.md': 'text/plain; charset=utf-8', '.csv': 'text/plain; charset=utf-8', '.json': 'text/plain; charset=utf-8', '.log': 'text/plain; charset=utf-8', '.html': 'text/plain; charset=utf-8', '.mp4': 'video/mp4', '.webm': 'video/webm', '.mov': 'video/quicktime', '.mp3': 'audio/mpeg' };
function stream(root, rel, res, inline) {
  ...
  const type = inline && INLINE[path.extname(t.abs).toLowerCase()];
  res.writeHead(200, { 'content-type': type || 'application/octet-stream', 'content-length': st.size, 'content-disposition': `${type ? 'inline' : 'attachment'}; filename*=UTF-8''${encodeURIComponent(path.basename(t.abs))}`, 'cache-control': 'no-store' });
  fs.createReadStream(t.abs).pipe(res);
}

// Several files / folders in one go: a zip streamed straight from `zip -r -` (nothing written to disk).
function zip(root, rels, res, name) {
  const ok = []; for (const r of rels) { const t = safe(root, r); if (t && t.rel) ok.push(t.rel); }
  if (!ok.length) { res.writeHead(400); return res.end('nothing to zip'); }
  const p = require('child_process').spawn('zip', ['-r', '-q', '-', '--', ...ok], { cwd: fs.realpathSync(root) });
  ...
  p.stdout.pipe(res); p.on('error', () => res.end()); res.on('close', () => { try { p.kill(); } catch {} });
}

// Delete files / folders (folders recursively). Never the root itself.
function remove(root, rels) {
  const done = [], failed = [];
  for (const r of rels) { const t = safe(root, r); if (!t || !t.rel) { failed.push(r); continue; } try { fs.rmSync(t.abs, { recursive: true, force: false }); done.push(t.rel); } catch (e) { failed.push(r + ': ' + e.message); } }
  return { ok: !failed.length, done, failed };
}

module.exports = { list, read, save, stream, zip, remove };
