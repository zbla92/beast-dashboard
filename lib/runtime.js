...
async function snapshot(projects) {
  ...
  for (const t of tmux) {
    ...
    sessions.push({ name: t.name, managed, project: projectId, cwd: t.cwd, created: t.created, attached: t.attached, activity: t.activity, ports: myPorts.map(p => ({ addr: p.addr, port: p.port, loopback: p.loopback, ephemeral: p.loopback && p.port >= 32768 })), ...treeStats(pids), pids: pids.length, cmd: rec?.cmd || null, label: rec?.name || (t.name.startsWith('bd_') ? t.name.split('__')[1] : t.name), current: main ? labelFromCmd(cmdline(main.pid), main.comm) : t.cmd, startedAt: rec?.startedAt || t.created, exitCode: null, running: true });
  }
  ...
  for (const p of ports) {
    ...
    external.push({ pid: p.pid, comm: pr.comm, label: labelFromCmd(fullCmd, pr.comm), cmd: fullCmd.replace(/\/home\/[^/]+\/dev\//g, '~/dev/').replace(/\/home\/[^/]+\/\.nvm\/versions\/node\/[^/]+\/bin\//g, '').slice(0, 160), project: projectId, cwd: cwd(p.pid), ports: myPorts, ...treeStats(tree), startedAt: startOf(pr) });
  }
