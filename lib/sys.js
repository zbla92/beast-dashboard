...
// long temperature history: one point a minute, 24 h, persisted so it survives a restart
const { readJson, writeJson, STATE } = require('./util');
const TFILE = require('path').join(STATE, 'temps.json');
let temps = null, lastTempAt = 0, tempsDirty = 0;
function tempsLoad() { if (!temps) { temps = readJson(TFILE, []); const cut = Date.now() - 24 * 3600e3; temps = temps.filter(x => x[0] > cut); } return temps; }
function tempsSample(st) {
  const now = Date.now(); if (now - lastTempAt < 60000) return; lastTempAt = now;
  const t = tempsLoad(); t.push([now, st.cpu.temp ?? null, st.gpu?.temp ?? null, st.temps?.nvme ?? null, Math.round(st.cpu.pct), st.gpu ? st.gpu.util : null, Math.round(st.mem.used / st.mem.total * 100)]); while (t.length && t[0][0] < now - 24 * 3600e3) t.shift();
  if (++tempsDirty >= 5) { tempsDirty = 0; try { writeJson(TFILE, t); } catch {} }
}
function tempsHistory() { return tempsLoad(); }
function latest() { return last; }
module.exports = { sample, history, latest , tempsSample, tempsHistory };
