...
function noteRun(tid, session) { const t = load().find(x => x.id === tid); if (!t) return; t.runs.push({ at: Date.now(), session }); if (t.status === 'todo') t.status = 'doing'; t.updatedAt = Date.now(); save(); }
...
module.exports = { all, add, update, remove, noteRun, prompt };
